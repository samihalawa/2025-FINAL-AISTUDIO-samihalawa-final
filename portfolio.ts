...
export const LINKEDIN_MEDIA_ASSETS: LinkedInMediaAsset[] = [
  ...
  },
  {
    ...
    proof: copy('111 valid Meet Recording media files and approximately 125.83 hours are inventoried. A full 111-row status index now exists: 25 indexed, 62 needs-review, 13 private-skip and 11 media-only-needs-transcript. The current sidecar pass includes 10 Google Docs for Chinese lessons, one IWAKY scope meeting, one Google-native Valerio/Umbramed advisory session, five local Valerio/Umbramed transcripts covering 11 media rows, and four root local chat sidecars.', '111 archivos multimedia válidos de Meet Recordings y aproximadamente 125,83 horas están inventariados. Ya existe un índice de estado completo de 111 filas: 25 indexadas, 62 pendientes de revisión, 13 privadas/no publicables sin redacción y 11 multimedia sin sidecar y pendientes de transcripción. El pase actual de sidecars incluye 10 Google Docs de clases de chino, una reunión de alcance IWAKY, una sesión Valerio/Umbramed de Google, cinco transcripciones locales de Valerio/Umbramed que cubren 11 filas multimedia y cuatro chats locales raíz.'),
  },
  {
    id: 'linkedin-post20',
    post: 'Post 20',
...
export const EVIDENCE_GAPS: EvidenceGap[] = [
  ...
  },
  {
    ...
    status: copy('111 valid media files and approximately 125.83 hours are verified at file level, with 271 notes/transcript/chat sidecars and 0 remaining timed-out media candidates. A full status TSV now covers all 111 rows; 12 Google sidecars, 5 local Valerio/Umbramed transcripts and 4 root local chats have been read. Full transcript/content review remains incomplete.', '111 archivos multimedia válidos y unas 125,83 horas están verificados a nivel de archivo, con 271 notas/transcripciones/chats auxiliares y 0 candidatos de media con timeout pendientes. Un TSV completo de estado cubre ya las 111 filas; se han leído 12 sidecars de Google, 5 transcripciones locales de Valerio/Umbramed y 4 chats locales raíz. La revisión completa de transcripciones/contenido sigue incompleta.'),
...
export const PROGRESSION_PLAN_STEPS: ProgressionPlanStep[] = [
  {
    ...
    currentState: copy('111 valid media files / approximately 125.83 hours are verified at file level; 271 sidecars are inventoried, 0 timed-out candidates remain, 12 Google sidecars plus 5 local Valerio/Umbramed transcripts plus 4 root local chats have been read and the full 111-row status TSV exists.', '111 archivos válidos / unas 125,83 horas están verificados a nivel de archivo; hay 271 sidecars inventariados, no quedan candidatos con timeout, se han leído 12 sidecars de Google más 5 transcripciones locales de Valerio/Umbramed más 4 chats locales raíz y existe el TSV completo de estado de 111 filas.'),
    nextProofAction: copy('Review the 62 remaining needs-review rows with sidecar leads first; return to the 11 media-only-needs-transcript rows only when transcript extraction or manual review is needed for a public claim.', 'Revisar primero las 62 filas pendientes de revisión restantes con sidecars; volver a las 11 filas multimedia sin sidecar solo cuando haga falta extracción de transcripción o revisión manual para un claim público.'),
