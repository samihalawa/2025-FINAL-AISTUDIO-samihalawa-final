...
export const PROGRESSION_PLAN_STEPS: ProgressionPlanStep[] = [
  {
    ...
    currentState: copy('111 valid media files / approximately 125.83 hours are verified at file level; 271 sidecars are inventoried, 0 timed-out candidates remain, 18 Google sidecars plus 7 local Valerio/Umbramed transcripts plus 8 local Valerio/Umbramed chat sidecars plus 4 root local chats have been read and the full 111-row status TSV exists.', '111 archivos válidos / unas 125,83 horas están verificados a nivel de archivo; hay 271 sidecars inventariados, no quedan candidatos con timeout, se han leído 18 sidecars de Google más 7 transcripciones locales de Valerio/Umbramed más 8 chats locales de Valerio/Umbramed más 4 chats locales raíz y existe el TSV completo de estado de 111 filas.'),
    nextProofAction: copy('Review the 55 remaining needs-review rows with sidecar leads first; return to the 11 media-only-needs-transcript rows only when transcript extraction or manual review is needed for a public claim.', 'Revisar primero las 55 filas pendientes de revisión restantes con sidecars; volver a las 11 filas multimedia sin sidecar solo cuando haga falta extracción de transcripción o revisión manual para un claim público.'),
