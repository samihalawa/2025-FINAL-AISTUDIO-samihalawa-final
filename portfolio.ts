...
export const LINKEDIN_MEDIA_ASSETS: LinkedInMediaAsset[] = [
  ...
  },
  {
    ...
    proof: copy('111 valid Meet Recording media files and approximately 125.83 hours are inventoried. A full 111-row status index now exists: 12 indexed, 78 needs-review, 10 private-skip and 11 media-only-needs-transcript. The current priority Chinese lesson sidecars are exported/read: 10 Google Docs, 7 substantive lessons and 3 low-content setup fragments; one IWAKY scope meeting and one Valerio/Umbramed advisory session are also content-mapped.', '111 archivos multimedia válidos de Meet Recordings y aproximadamente 125,83 horas están inventariados. Ya existe un índice de estado completo de 111 filas: 12 indexadas, 78 pendientes de revisión, 10 privadas/no publicables sin redacción y 11 multimedia sin sidecar y pendientes de transcripción. Los sidecars prioritarios actuales de clases de chino están exportados/leídos: 10 Google Docs, 7 clases sustantivas y 3 fragmentos de configuración de bajo contenido; una reunión de alcance IWAKY y una sesión de asesoría Valerio/Umbramed también están mapeadas a contenido.'),
  },
  {
    id: 'linkedin-post20',
    post: 'Post 20',
...
export const EVIDENCE_GAPS: EvidenceGap[] = [
  ...
  },
  {
    ...
    status: copy('111 valid media files and approximately 125.83 hours are verified at file level, with 271 notes/transcript/chat sidecars and 0 remaining timed-out media candidates. A full status TSV now covers all 111 rows; twelve Google sidecars have been exported/read: 10 Chinese lesson docs, 1 IWAKY and 1 Valerio/Umbramed. Full transcript/content review remains incomplete.', '111 archivos multimedia válidos y unas 125,83 horas están verificados a nivel de archivo, con 271 notas/transcripciones/chats auxiliares y 0 candidatos de media con timeout pendientes. Un TSV completo de estado cubre ya las 111 filas; se han exportado/leído 12 sidecars de Google: 10 documentos de clases de chino, 1 de IWAKY y 1 de Valerio/Umbramed. La revisión completa de transcripciones/contenido sigue incompleta.'),
...
export const PROGRESSION_PLAN_STEPS: ProgressionPlanStep[] = [
  {
    ...
    currentState: copy('111 valid media files / approximately 125.83 hours are verified at file level; 271 sidecars are inventoried, 0 timed-out candidates remain, 12 Google sidecars have been read and the full 111-row status TSV exists.', '111 archivos válidos / unas 125,83 horas están verificados a nivel de archivo; hay 271 sidecars inventariados, no quedan candidatos con timeout, se han leído 12 sidecars de Google y existe el TSV completo de estado de 111 filas.'),
    nextProofAction: copy('Review the 78 needs-review rows with sidecar leads first; return to the 11 media-only-needs-transcript rows only when transcript extraction or manual review is needed for a public claim.', 'Revisar primero las 78 filas pendientes de revisión con sidecars; volver a las 11 filas multimedia sin sidecar solo cuando haga falta extracción de transcripción o revisión manual para un claim público.'),
