...
const headings: Record<LanguageCode, { eyebrow: string; title: string; body: string; all: string; proof: string }> = {
  en: { eyebrow: 'Flagship evidence', title: 'Real products, shown at the user layer.', body: 'Screenshots come from current product surfaces when the visible claims are safe to publish; otherwise the card stays explicitly evidence-led.', all: 'See every curated project', proof: 'Open evidence' },
  es: { eyebrow: 'Evidencia principal', title: 'Productos reales, mostrados en la capa del usuario.', body: 'Las capturas proceden de productos actuales cuando sus afirmaciones visibles son publicables; si no, la tarjeta conserva el límite de evidencia.', all: 'Ver todos los proyectos curados', proof: 'Abrir evidencia' },
  fr: { eyebrow: 'Preuves phares', title: 'Des produits réels, visibles au niveau utilisateur.', body: 'Les captures viennent des produits actuels lorsque leurs affirmations visibles sont publiables; sinon la carte reste centrée sur la preuve.', all: 'Voir tous les projets sélectionnés', proof: 'Ouvrir la preuve' },
  zh: { eyebrow: '旗舰证据', title: '真实产品，在用户层展示。', body: '当页面中的可见声明适合公开时使用当前产品截图；否则卡片明确保留证据边界。', all: '查看全部精选项目', proof: '打开证据' }
};
